...
namespace SKCell
{
    ...
    public class SKGridLayer : MonoBehaviour
    {
        ...
        public Vector2Int GetCellFromWorldPos(Vector3 pos)
        {
            return grid.PositionToCell(pos);
        }
        public List<Vector2Int> GetCellsFromWorldPos(Vector3 pos1, Vector3 pos2)
        {
            return grid.GetCells(pos1, pos2);
        }
        public List<Vector2Int> GetCellsFromCoordinate(Vector2Int cell1, Vector2Int cell2)
        {
            return grid.GetCells(cell1.x, cell2.x, cell1.y, cell2.y);
        }
        public Vector3 GetWorldPosFromCell(Vector2Int cell)
        {
            return grid.CellToPosition(cell.x, cell.y);
        }
        #endregion
        ...
        public bool GetCellOccupancy(int x, int y)
        {
            if (!CheckCellValidity(x, y))
                return true;

            return grid.occupied[x, y];
        }
